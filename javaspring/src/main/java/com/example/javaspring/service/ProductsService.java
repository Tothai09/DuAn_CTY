...
public class ProductsService {
    ...
    public String postProduct(ProductsDto productsDto){
        Products products = productsRepository.findByNameProduct(productsDto.getNameProduct());
        String test;
        if(products != null){
            test = "Đã tồn tại";
        }
        else{
            if(categoryRepository.findByNameCategory(productsDto.getCategory())!= null){
                Products productPost = productMapper.toProduct(productsDto);
                Category category = categoryRepository.findByNameCategory(productsDto.getCategory());
    ...
    public String putProduct(ProductsDto productsDto, long id){
        ...
        String test;
        if(productsOptional.isPresent() && categoryRepository.findByNameCategory(productsDto.getCategory())!= null){
